import { getStorageInfrastructure } from './infrastructure';

export const createRepository = async (): Promise<StorageRepository | null> => {
  const infra = await getStorageInfrastructure();
  if (!infra) return null;
  const { local, sync } = infra;
  ...
  const getProfile = async (id: string) => {
    const { v3 } = await infra.sync.get({ v3: { profiles: {} } });
    return v3.profiles[id] ?? undefined;
  };
  const setProfile = async (profile: ProfileWithMetaData) => {
    const { v3: curr } = await infra.sync.get({ v3: { profiles: {} } });
